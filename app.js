...
app.get( '/webslidecontrol.js', function( req, res) {
  ...
  socketStraem.on('close', function() {
    ...
    jqueryStraem.on('close', function() {
      var buffer = new Buffer('var user = ' + user +'; ');
...
// Start server
io = sio.listen( http.createServer( app ).listen( app.get('port'), function() {
...
io.sockets.on( 'connection', function( socket ) {
  ...
  socket.on('connect', function(data) {
    if (data.part && data.part === 'client') {
      if ( !sliders[data.user]) {
        sliders[data.user] = [];
      }
      sliders[data.user].push(socket.id);
    } else {
      if ( !controls[data.user]) {
        controls[data.user] = [];
      }
      controls[data.user].push(socket.id);
    } 

    console.log(data.part + ': ' + data.user + ' add!');
    console.log(sliders);
  ...
  socket.on('start', function (data) {
      ...
      var len = sliders[data].length;
      
      for (var i = 0; i<len; i++) {
        io.sockets.socket(sliders[data][i]).emit('start', data);
      }
  });
  ...
  socket.on('end', function (data) {
      ...
      var len = sliders[data].length;
      for (var i = 0; i<len; i++) {
        io.sockets.socket(sliders[data][i]).emit('end', data);
      }
  });
  ...
  socket.on('pre', function (data) {
      ...
      var len = sliders[data].length;
      for (var i = 0; i<len; i++) {
        io.sockets.socket(sliders[data][i]).emit('pre', data);
      }
  });
  ...
  socket.on('next', function (data) {
      ...
      for (var i = 0; i<len; i++) {
        io.sockets.socket(sliders[data][i]).emit('next', data);
      }
  });
});
